...
- **제목**: 50자 이내, `feat:` `fix:` `refactor:` `docs:` `chore:` 등 타입을 명시하고
  무엇을 구현/수정했는지만 간단히 쓴다.
- **본문**: 주요 컴포넌트/기능만 bullet으로 나열한다. 최대 5~7개, 각 항목 한 줄.
- **제외할 내용**: 기술 스택 세부사항, 파일명/경로, 개수·통계, 구현 방법론,
  하위 세부사항 등 코드 리뷰에서 확인 가능한 모든 내용.
- 핵심 원칙: **"코드 리뷰에서 볼 수 있는 내용은 커밋 메시지에 쓰지 않는다."**

```
feat: 방문정비 예약 상세 페이지 구현

- UserInfo: 예약자 정보
- MechanicInfo: 정비사 정보
- ReservationInfo: 예약 및 차량 정보
```
