...
전체 목표와 마일스톤은 [docs/GOAL.md](docs/GOAL.md), 아키텍처 기준은
[docs/ARCHI.md](docs/ARCHI.md)를 따른다.
...
- [ ] Claude Code 플러그인 패키징 (스킬 + 뷰어 배포 방식)
...
2. **Recap 표준 골격** — UI 와이어프레임 헤드라인 → 1~3문단 서사 →
   스키마/API 변경 → 변경 파일 트리 → "Key changes" diff 탭 순서.
3. **Budget** — Key-change 탭 3~8개, 발췌는 탭당 ~150줄 미만, 타이틀 ~70자 이하.
   철저함을 핑계로 budget을 넘기지 않는다. 3~8은 상한 중심 기준이며, diff
   후보 파일이 3개 미만이면 있는 만큼만 만들고 0개면 탭을 생략한다.
...
- 커밋·PR 등 GitHub 메타데이터에 AI attribution(`Co-Authored-By`,
  에이전트 라벨 등)을 넣지 않는다.
- PR 본문은 `.github/PULL_REQUEST_TEMPLATE.md` 구조를 따른다. `gh pr create`처럼
  본문을 직접 지정하는 경로에서는 템플릿이 자동 삽입되지 않으므로 직접 준수한다.
...
- 커밋 메시지와 PR 제목/본문은 한글로 작성한다.
- 커밋 메시지는 다음 형식을 사용한다.

```text
feat|fix|refactor|docs|chore(스코프): 커밋 메시지

- 커밋 내용 요약 1
- 커밋 내용 요약 2
```

- 스코프는 선택이며 변경 영역을 쓴다 (예: `viewer`, `skills`, `launcher`).
- 본문 bullet은 주요 변경만 간결하게 나열한다.
...
- **포트는 previs 전용 대역 `47738~47801`만 사용한다** — `47738 + (경로 해시 % 64)`.
  3000, 5173, 8080 등 흔한 개발 포트를 절대 사용하지 않는다 (dev 서버 기본 포트
  설정도 이 대역으로 덮어쓴다). 유도된 포트가 다른 프로세스에 점유돼 있으면
  대역 내에서 +1 상향 탐색한다.
