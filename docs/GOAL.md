...
### M4. 에이전트 스킬 — /previs-plan · /previs-recap — ✅ 완료 (v0.5.0)

에이전트가 문서를 생성하는 파이프라인.

- `/previs-plan` (v0.4.1): 계획을 블록 문서로 생성 (승인 게이트 워크플로우)
- `/previs-recap` (v0.5.0): git diff에서 기계적으로 블록 도출 — `@previs/recap`
  CLI가 file-tree·diff 블록을 산출해 Grounding·Budget·시크릿 마스킹을 코드로
  강제하고, 서사·와이어프레임만 스킬이 작성 (하이브리드). Recap 표준 골격,
  Skip 조건 포함
- 로컬 발행: `.previs/` 문서 폴더 → 뷰어가 로드
- 완료 기준: 실제 diff로 recap 생성 → 뷰어 확인 E2E 시나리오 통과 ✅
  (`v0.3.0..v0.4.1` 실범위 도출 → 블록 조립 → doc:validate 통과)

### M5. 로컬 런처 — 싱글턴 서버 — ✅ 완료 (v0.6.0)

- `@previs/launcher`: 재사용 우선 기동(`/api/viewer-info` 정체 확인 → 원자적
  락 → 기동), 락 없이 뜬 동일 뷰어 채택으로 중복 기동 방지
- `targetKey`=canonical docsDir로 포트·락·재사용 정체 통일(`PREVIS_DOCS_DIR`
  오버라이드), detached 프로세스 그룹 정리, 유휴 30분 자동 종료, 전용 대역
  (`47738~47801`) 해시 포트 — 흔한 개발 포트 미사용
- 두 스킬(`/previs-plan`·`/previs-recap`)이 `pnpm viewer:up` 호출로 연동
- 완료 기준: 다중 세션 동시 호출에서 프로세스 1개 보장 ✅ (락·채택 단위·통합
  테스트 36건 + 동시 3콜 → 리스너 1개 E2E 검증)

**→ M5까지 완료로 "개인 도구"로 실사용 가능한 1차 완성 달성.**
...
- 사전 결정 필요: 배포 라이선스 (공개 배포 전 확정)
- npm 패키지명 `previs` 확보 — 2026-07 기준 미점유 확인, 공개 계획 확정 시 조기 선점
- AGENTS.md "추후 계획" 항목
