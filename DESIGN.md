...
<!-- 원본: VoltAgent/awesome-design-md (MIT) — design-md/minimax/DESIGN.md 기반. previs에 맞게 적용 지침을 추가함. -->

## previs 적용 지침

이 문서는 **뷰어 앱 UI(크롬)에만** 적용한다. 아래 경계를 반드시 지킨다.

- **적용 대상**: 내비게이션, 문서 목록, 다이얼로그, 버튼, 배지, 테이블 등 previs 앱 자체의 UI.
- **적용 금지**: plan/recap 문서의 블록 콘텐츠 렌더링 — 와이어프레임(`--wf-*` 토큰 체계),
  다이어그램, diff 뷰는 콘텐츠 규칙(AGENTS.md)의 손그림 미학을 따르며 이 문서의 브랜드
  토큰을 입히지 않는다.
- **한글 폰트**: DM Sans는 한글 글리프가 없다. 폰트 스택은
  `"DM Sans", "Pretendard Variable", sans-serif`로 구성해 한글은 Pretendard가 렌더링한다.
- **다크 모드**: 이 명세는 라이트 캔버스 기준 분석이라 다크 토큰이 없다. 구현 시
  semantic 토큰(CSS 변수)으로 변환하면서 다크 변형을 함께 정의한다.
- **미사용 컴포넌트**: `promo-banner`, `hero-band-marketing` 등 마케팅 전용 컴포넌트는
  previs에서 사용하지 않는다.
- **그라디언트 활용**: 모델 릴리스 카드의 그라디언트 정체성(coral/magenta/blue/purple)은
  plan/recap 문서 카드의 식별색으로 전용한다 — 문서마다 고유한 시각 정체성을 부여.
