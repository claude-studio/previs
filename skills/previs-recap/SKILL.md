...
이번 스킬의 기본 발행 대상은 previs 리포지토리 루트의 `.previs/`다. 다른 프로젝트의
문서를 확인해야 하면 `PREVIS_DOCS_DIR` 환경변수로 서빙 디렉토리를 오버라이드할 수
있다.

## 6. 뷰어 확인

다음 명령으로 뷰어를 재사용 우선 기동한다. 기존 서버가 있으면 동일한 로컬 URL을
반환하고, 없으면 서버를 백그라운드로 기동한다. 출력된 URL을 열어 발행 문서를
확인하도록 안내한다.
